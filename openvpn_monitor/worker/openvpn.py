...
from openvpn_monitor.worker.data import SessionData, SessionBytes
...
class OVPNMonitor(multiprocessing.Process):
    ...
    def status_raw(
        ...
        try:
            ...
            print(
                f"{self.host}: Got {e.__repr__()} when tried to fetch data "
    ...
    def run(
        ...
        print(f'Started monitoring for host {self.host}:{self.port}', flush=True)
        timestamp = int(time.time())
        status = {}
